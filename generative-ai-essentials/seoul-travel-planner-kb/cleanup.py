...
삭제 순서에 의미가 있다. 참조하는 쪽을 먼저 지운다:
    S3 이벤트 알림 -> Lambda -> 데이터 소스 -> Knowledge Base
    -> OpenSearch 컬렉션 -> 접근/보안 정책 -> S3 객체·버킷 -> IAM 역할
...
# 이 실습이 만든 리소스 중 kb_info.json에 기록되지 않는 것들.
# setup 스크립트의 상수와 같은 값이어야 한다. 다른 값을 쓰면 지우지 못한다.
FUNCTION_NAME = "travel-kb-auto-sync"
LAMBDA_ROLE_NAME = "travel-kb-auto-sync-lambda-role"
...
def main() -> int:
    ...
    log("1. S3 이벤트 알림과 Lambda")
    bucket = info.get("bucket")
    if bucket and exists(f"S3 버킷 {bucket}", lambda: s3.head_bucket(Bucket=bucket)):
        attempt(
            f"S3 이벤트 알림 {bucket}",
            lambda: s3.put_bucket_notification_configuration(
                Bucket=bucket, NotificationConfiguration={}
            ),
            dry,
        )
    if exists(
        f"Lambda 함수 {FUNCTION_NAME}",
        lambda: lam.get_function(FunctionName=FUNCTION_NAME),
    ):
        attempt(
            f"Lambda 함수 {FUNCTION_NAME}",
    ...
    delete_role(iam, LAMBDA_ROLE_NAME, dry)
