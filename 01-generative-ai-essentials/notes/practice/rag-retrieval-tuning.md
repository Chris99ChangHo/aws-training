...
수업 필기 원본은 별도 문서(Google Docs)로 관리한다. 필기에 없던 사실을
정리본에 섞지 않기 위해서다.
...
> `[실측]` **일반 명사 질의 두 건에서는 개선이 없었다.** seoul 1/3, restaurant 1/3 —
> 기본 검색과 동일했다. (후속 실측: 고유 키워드 질의에서는 결과가 다르다 —
> "확인하지 못한 것" 절 참고)
...
- ~~하이브리드가 이기는 질의를 실제로 측정하지 못했다~~ → **이후 측정됨.**
  `seoul-travel-planner-kb` 실습에서 고유 키워드가 포함된 질의
  ("후원 부용지 연경당")로 다시 테스트한 결과, `SEMANTIC`은 정답 문서(창덕궁)가
  상위 5위 밖이었고 `HYBRID`는 본문의 고유 키워드가 매칭 보너스로 작용해
  1위로 올라왔다. 이 노트 3절의 "질의에 고유 명사·식별자가 있을 때
  하이브리드를 선택한다"는 판단은 이 실측으로 뒷받침된다. 다만 위 1~4절의
  실측 표는 "회식하기 좋은 한식당" 같은 일반 명사 질의만 다루므로, 별도로
  갱신하지 않고 이 항목에 후속 실측을 기록해 둔다. 자세한 내용은
  [`seoul-travel-planner-kb/README.md` 트러블슈팅 1번](../../seoul-travel-planner-kb/README.md)을 본다.
