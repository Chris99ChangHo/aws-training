---
name: streamlit-ui
description: Streamlit 앱의 UI를 만들거나 고칠 때 사용한다. 색상 대비(WCAG 2.1 AA), 라이트/다크 테마 구성, 커스텀 CSS 주입, 실행 검증 절차. "다크모드가 깨진다", "테마 색 바꿔줘", "챗봇 UI 만들어줘" 같은 요청에 해당한다.
---

# Streamlit UI 작성 규칙

UI 디자인 자체(카드 스타일, 간격, 타이포그래피 체계)는 업계 표준이라 부를
단일 기준이 없다. 여기에는 **출처가 명확한 규칙만** 넣는다.
...
- README 기술 스택 배지 색상 규칙은 `readme-audit` 스킬에 있다
  (배지는 Streamlit 실습뿐 아니라 `agents/`에서도 쓰므로 분리했다).
