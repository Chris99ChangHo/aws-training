...
배포할 것이 없다. 이 에이전트는 클론해서 스크립트를 돌리는 방식으로 소비되고,
학습용 실습은 학습자가 자기 계정에 직접 만든다(자동 배포하면 남의 계정에
과금이 생긴다). 아무것도 배포하지 않는 `deploy` 잡을 넣어 이름만 CI/CD로
만들지 않았다.

대신 **SARIF를 GitHub 코드 스캐닝에 올린다.** 그 전까지 SARIF 상호운용성은
*우리 게이트가 읽는 것*만 확인된 상태였고, 외부 소비자가 받아주는지는
검증되지 않았다.

수락 확인 (`code-scanning/analyses`):

| category | tool | results | rules |
|---|---|---|---|
| `agents` | Trivy | 0 | 0 |
| `agents` | Semgrep OSS | 0 | **725** |
| `repo` | Trivy | **12** | 7 |

두 범위를 `category`로 구분했으므로 서로 덮어쓰지 않고, GitHub이 도구별로
run을 갈라 읽었다. **GitHub이 독립적으로 룰 725개를 보고한 것**은 이 문서의
룰셋 수치가 로컬 측정만이 아니라 외부에서 교차 확인됐다는 뜻이다.

등급 분류도 일치한다. 코드 스캐닝 알림은 high 5 / medium 4 / low 3 = 12건이고,
`gate.py`의 로컬·CI 출력과 **등급별로 같다.** 세 주체가 독립적으로 같은 분류에
도달한 것이므로, 게이트의 severity 해석 순서(`security-severity` → 룰
properties → `defaultConfiguration.level` → `result.level`)가 GitHub의 자체
구현과 같은 결과를 낸다는 실증이다.

업로드는 `if: always()`로 돈다. 게이트가 차단했을 때가 결과를 보고 싶은
시점이고, 그때 건너뛰면 Security 탭이 마지막 통과 상태로 남아 실제보다 깨끗해
보인다.
